...
3 王五
    ...
    >
      <el-form ref="form" :model="removeInfo" label-width="80px" size="mini">
        <el-form-item label="重製選項">
          <el-radio-group v-model="removeInfo.type">
            <el-radio border :label="0">重設全部數據</el-radio>
            <el-radio border :label="1">重設抽獎配置</el-radio>
            <el-radio border :label="2">重設名單</el-radio>
            <el-radio border :label="3">重設照片</el-radio>
            <el-radio border :label="4">重設抽獎結果</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="resetConfig">確定重設</el-button>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if (this.remain <= 0) {
        return this.$message.error('該獎項剩於人數不足');
      }
      if (this.form.mode === 99) {
        ...
        if (this.form.qty > this.remain) {
          return this.$message.error('本次抽獎人數已抄過本獎項的剩餘人數');
        }
      }
